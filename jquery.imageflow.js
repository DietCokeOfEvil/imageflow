...
function _imageFlow(container, params) {
    ...
    containedImages.each(function() {
        ...
        if (currentWidth + image.outerWidth(true) >= rowWidth) {
            ...
            _adjustRow(rowWidth, currentWidth, previousRowItems);
            image.before("\n");
            currentWidth = 0;
...
function _adjustRow(maxRowWidth, currentRowWidth, previousRowItems) {
    if (0 >= previousRowItems.length) return currentRowWidth;
    
    var percent = maxRowWidth / currentRowWidth;
    ...
    lastItem.css("width", maxRowWidth - adjustedRowWidth - xBoxPadding);
    adjustedRowWidth += lastItem.outerWidth(true);
    
    return adjustedRowWidth;
}
